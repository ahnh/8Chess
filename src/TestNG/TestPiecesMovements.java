...
public class TestPiecesMovements {

	@BeforeTest
	public void testBeforeTest() {
		// setup board etcs
		System.out.println("PiecesMovementTest");
	}
	// Test BK movement
	@Test
	public void BishopKnightMovement() {
		...
		for (int y = 0; y < boardSize; y++)
			for (int x = 0; x < boardSize; x++) {
				int y2 = boardSize / 2; // Center Board is used to get all
										// possible directions
				int x2 = boardSize / 2; // Center Board is used to get all
										// possible directions
				...
				int hDifference = Math.max(start.x, end.x)
						- Math.min(start.x, end.x);
				int vDifference = Math.max(start.y, end.y)
				...
				if ((hDifference == 2 && vDifference == 1)
					...
					expected = true;
						...
						&& (start.x != end.x && start.y != end.y)) // Only
																	// Diagonals
																	// Allowed
					expected = true;
				else
				...
				Assert.assertEquals(actual, expected,
						"BishopKnight Performed an Illegal Move:(" + start.x
								+ "," + start.y + "->" + end.x + "," + end.y);
			}
	}
	...
	public void QueenKnightMovement() {
		...
		for (int y = 0; y < boardSize; y++)
			for (int x = 0; x < boardSize; x++) {
				...
				// Bishop Movement
				else if (Math.abs(end.x - start.x) == Math.abs(end.y - start.y)) // Only
																					// Diagonals
																					// Allowed
					expected = true;
				...
				Assert.assertEquals(actual, expected,
						"QueenKnight Performed an Illegal Move:(" + start.x
								+ "," + start.y + "->" + end.x + "," + end.y);
			}
	}

	// This will use queen movement as testing since the queen can move anywhere
	// the king can,
	// plus the king must move like the queen on the first turn, 2 birds 1
	// stone.
	@Test
	public void CheshireKingMovement() {
		...
		for (int y = 0; y < boardSize; y++)
			for (int x = 0; x < boardSize; x++) {
				...
				Assert.assertEquals(actual, expected,
						"King Performed an Illegal Move:(" + start.x + ","
								+ start.y + "->" + end.x + "," + end.y);
			}
		...
		for (int y = 0; y < boardSize; y++)
			for (int x = 0; x < boardSize; x++) {
				...
				if (x2 - x == 0 && y2 - y == 0)
					...
					expected = true;
				else if (y2 - y == 0 && Math.abs(x2 - x) <= 2)// King CAN move 2
																// to the left
																// and right on
																// castle
					expected = true;
	...
	public void QueenMovement() {
		...
		for (int y = 0; y < boardSize; y++)
			for (int x = 0; x < boardSize; x++) {
				...
				Assert.assertEquals(actual, expected,
						"Rook Performed an Illegal Move:(" + start.x + ","
								+ start.y + "->" + end.x + "," + end.y);
			}
	...
	public void KnightMovement() {
		...
		for (int y = 0; y < boardSize; y++)
			for (int x = 0; x < boardSize; x++) {
				...
				Assert.assertEquals(actual, expected,
						"Knight Performed an Illegal Move:(" + start.x + ","
								+ start.y + "->" + end.x + "," + end.y);
			}
	...
	public void RookKnightMovement() {
		...
		for (int y = 0; y < boardSize; y++)
			for (int x = 0; x < boardSize; x++) {
				...
				Assert.assertEquals(actual, expected,
						"RookKnight Performed an Illegal Move:(" + start.x
								+ "," + start.y + "->" + end.x + "," + end.y);
			}
	...
	public void BishopMovement() {
		...
		for (int y = 0; y < boardSize; y++)
			for (int x = 0; x < boardSize; x++) {
				...
				Assert.assertEquals(actual, expected,
						"Bishop Performed an Illegal Move:(" + start.x + ","
								+ start.y + "->" + end.x + "," + end.y);
			}
	...
	public void PawnMovement() {
		...
		for (int y = 0; y < boardSize; y++)
			for (int x = 0; x < boardSize; x++) {
				...
				else if ((y2 - y) > 2 || (y2 - y) < 0) // Pawns Are only Allowed
														// 2 Up
					expected = false;
				...
				Assert.assertEquals(actual, expected,
						"Pawn Performed an Illegal Move:(" + start.x + ","
								+ start.y + "->" + end.x + "," + end.y);
			}
	...
	public void HobbitPawnMovement() {
		...
		for (int y = 0; y < boardSize; y++)
			for (int x = 0; x < boardSize; x++) {
				...
				Assert.assertEquals(actual, expected,
						"HobbitPawn Performed an Illegal Move:(" + start.x
								+ "," + start.y + "->" + end.x + "," + end.y);
			}
	...
	public void StationaryKing() {
		...
		for (int y = 0; y < boardSize; y++)
			for (int x = 0; x < boardSize; x++) {
				...
				Assert.assertEquals(actual, expected,
						"StationaryKing Performed an Illegal Move:(" + start.x
								+ "," + start.y + "->" + end.x + "," + end.y);
			}
	...
	public void JediKnightMovement() {
		...
		for (int y = 0; y < boardSize; y++)
			for (int x = 0; x < boardSize; x++) {
				...
				Assert.assertEquals(actual, expected,
						"JediKnight Performed an Illegal Move:(" + start.x
								+ "," + start.y + "->" + end.x + "," + end.y);
			}
	}
