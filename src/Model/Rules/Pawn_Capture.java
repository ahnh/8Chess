...
public class Pawn_Capture extends Rule {
	...
	public int checkMove(Board board, Stack<Move> moves) {
		...
		if (currentMove.getPiece().getName().equalsIgnoreCase("pawn")) {
			// If the Pawn is moving diagonally then we need to check, otherwise
			// we don't care
			if ((currentMove.getDistanceX() == 1)
					&& (currentMove.getDistanceY() == 1)) {
				...
				else if ((board.getTile(currentMove.getEnd()) != null)
