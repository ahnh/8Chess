...
public class Hobbit_Move extends Rule {
	...
	public int checkMove(Board board, Stack<Move> moves) {
		...
		if (pieceInEndSpace == null) {
			...
			if (currentMove.getStart().x != currentMove.getEnd().x
					&& currentMove.getStart().y != currentMove.getEnd().y)
				return Rule.INVALID_MOVE;
		} else if (pieceInEndSpace.getTeam() == currentMove.getPiece()
				.getTeam()) {
