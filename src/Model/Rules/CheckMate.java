...
public class CheckMate extends Rule {
	...
	boolean IsCheckMate(Board board, int currentTeam) {
		...
		// System.out.println("[noCheck] Available moves by Team["+currentTeam+"]:"+
		// team_ValidMovePool.size());

		Check check = new Check();
		boolean isCheck = check.isKingInCheck(board, currentTeam);

		// System.out.println("KING CHECK = "+isCheck);
		// Cycle through list, if a move exists that does not leave the king in
		// check
		...
		if (isCheck) {
			...
			// If no valid move exists, and the king is in check, then this
			// person is in checkmate
						...
						if (comparePiece.getTeam() == team) {
							...
							for (int y2 = 0; y2 < board.getHeight(); y2++)
								for (int x2 = 0; x2 < board.getWidth(); x2++) {
									...
									Move move = new Move(point, p2,
											comparePiece);
									// If the piece can physically make the move
									if ((comparePiece.checkDestination(move))) 
									{
										// Check if the move can be made from
										// the collisions point of view
										// Create the Stack to Check on
										// Collision Rule
										Stack moveStack = new Stack();
										moveStack.add(move);
										if (collMove
												.checkMove(board, moveStack) == Rule.VALID_MOVE) {
